...
export default function DevotionalSeriesList({ onEdit }: Props) {
  ...
  return (
    <>
      ...
      {/* ── New series modal — reference pattern shell ───────────────────── */}
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-sm">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
              <div className="space-y-4">
                <p className="text-[13px] text-gray-500 leading-relaxed">
                  Short daily devotional series delivered to members one day at a time.
                </p>
                ...
                <div>
                  ...
                  <select
                    value={newType}
                    onChange={e => setNewType(e.target.value)}
                    className="w-full px-3.5 py-2.5 text-[13px] text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-300 bg-white"
                  >
                    {Object.entries(TYPE_LABELS).map(([v, l]) => (
                      <option key={v} value={v}>{l}</option>
                    ))}
