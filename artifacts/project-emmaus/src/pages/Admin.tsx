...
export default function Admin() {
      ...
      case 'content-studio':
        return (
          <ContentStudio
            ...
            onOpenLegacyEditor={(journeyId) => navigate({ section: 'content-studio', subView: 'studio-editor', journeyId })}
          />
        );
      case 'bible-study':
    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[14px] text-left transition-colors ${
          active ? 'bg-teal-50 text-teal-800 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
        }`}
      >
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <nav className={`flex flex-col h-full ${mobile ? '' : 'border-r border-gray-200'} bg-white`} aria-label="Admin navigation">
      ...
      <div className="flex-1 px-3 py-4 overflow-y-auto">
        <div className="space-y-0.5">{PRIMARY_NAV.map(item => <NavButton key={item.id} {...item} />)}</div>

        <div className="mt-6 mb-2 px-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Create and manage content</div>
        <div className="space-y-0.5">{MINISTRY_TOOLS.map(item => <NavButton key={item.id} {...item} />)}</div>

        <details className="mt-6 group">
          <summary className="px-3 py-2 text-[11px] font-medium text-gray-400 cursor-pointer hover:text-gray-600">Administration tools</summary>
          <div className="space-y-0.5 mt-1">{SYSTEM_TOOLS.map(item => <NavButton key={item.id} {...item} />)}</div>
        </details>
      </div>

      <div className="px-3 py-4 border-t border-gray-100 space-y-1">
        {isDemoMode && <div className="mx-3 mb-2 px-2 py-1 bg-amber-50 border border-amber-200 rounded text-[11px] text-amber-700 font-medium text-center">DEMO MODE</div>}
        <button onClick={() => setLocation('/walk')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[14px] text-gray-500 hover:bg-gray-50 hover:text-gray-900">
          <ArrowLeft size={16} className="text-gray-400" /> Back to App
        </button>
        <button onClick={async () => { await signOut(); setLocation('/'); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[14px] text-gray-500 hover:bg-gray-50 hover:text-gray-900">Sign out</button>
      </div>
    </nav>
  );
    ...
    <AdminProvider>
      <MediaStudioProvider>
        <div className="flex h-[100dvh] bg-gray-50 overflow-hidden">
          <aside className="hidden md:flex w-56 flex-col flex-shrink-0"><Sidebar /></aside>
          {sidebarOpen && (
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            <header className="md:hidden flex items-center h-12 px-4 bg-white border-b border-gray-200 flex-shrink-0">
              <button onClick={() => setSidebarOpen(true)} className="p-1 text-gray-500" aria-label="Open navigation"><Menu size={20} /></button>
              <span className="ml-3 font-medium text-[14px] text-gray-800">{sectionLabel[nav.section]}</span>
            </header>
