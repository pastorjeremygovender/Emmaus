    ...
    <>
      <Card className="border-amber-200/60 dark:border-amber-800/40 bg-amber-50/40 dark:bg-amber-950/15">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {sermon.listenAvailable && sermon.audioUrl && (
            <button onClick={() => setPlayerOpen(true)} className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-2 text-[12px] font-semibold text-foreground">
              <Headphones size={13} /> Listen
            </button>
          )}
          {sermon.listenAvailable && !sermon.audioUrl && sermon.listenPath && (
      ...
      {sermon.audioUrl && (
        <SermonAudioPlayer
          ...
          startSeconds={sermon.relativeStartSeconds}
          title={sermon.title}
