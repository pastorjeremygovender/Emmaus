/**
 * Seed script — insert demo journeys into Postgres if they don't exist.
 *
 * Run from the api-server directory:
 *   node --experimental-strip-types src/scripts/seed-journeys.ts
 *
...
const JOURNEYS = [
  {
    ...
    tags: ["daily", "jesus", "beginner", "core"],
    durationDays: 7,
    status: "Published",
    xpReward: 70,
  },
...
async function seed() {
  ...
  for (const j of JOURNEYS) {
    ...
    if (existing.length > 0) {
      console.log(`  Journey '${j.id}' already exists — skipping.`);
      continue;
    }
    const now = new Date();
  ...
  for (const s of STEPS) {
    ...
    if (existing.length > 0) {
      console.log(`  Step ${s.journeyId} day ${s.day} already exists — skipping.`);
      continue;
    }
    const now = new Date();
    await db.insert(journeyStepsTable).values({
      ...
      scriptureReferences: [],
      content: {},  // legacy JSONB left empty
      status: "draft",
      createdAt: now,
      updatedAt: now,
    });
    console.log(`  Created step: ${s.journeyId} day ${s.day}`);
  }
