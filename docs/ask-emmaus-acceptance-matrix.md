...
| Golden unsupported-claim evaluation | Passed with isolated database-backed fixtures | The same multi-user acceptance command emits `ASK_EMMAUS_GOLDEN_EVALUATION` with relevance decisions and routing/retrieval/total latency for canonical actions, Bible references, vague prompts, pastoral questions, sermon matches, and unsupported/client-context claims |
